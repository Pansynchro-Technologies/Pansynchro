    ...
    public class MSSQLReader : IDbReader
    {
        ...
        private async Task<IDataReader> FullSyncReader(StreamDefinition stream)
        {
            ...
            var columnList = rcf?.Length > 0
                ...
                : stream.NameList;
            var columns = string.Join(", ", columnList.Select(s => '[' + s + ']'));
            var sql = $"select {columns} from [{stream.Name.Namespace}].[{stream.Name.Name}]";
            if (rcf?.Length > 0) {
                var order = stream.Identity?.Length > 0 ? rcf.Concat(stream.Identity.Except(rcf)) : rcf;
                sql = $"{sql} order by {string.Join(", ", order.Select(s => '[' + s + ']'))}";
            }
            var query = new SqlCommand(sql, _conn) { CommandTimeout = 0 };
