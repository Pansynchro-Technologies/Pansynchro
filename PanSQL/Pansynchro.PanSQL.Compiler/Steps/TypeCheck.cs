...
namespace Pansynchro.PanSQL.Compiler.Steps
{
	internal class TypeCheck : VisitorCompileStep
	{
		...
		public override void OnSqlStatement(SqlTransformStatement node)
		{
			...
			var output = ((VarDeclaration)node.Output.Declaration).Stream.Fields;
			...
			if (node.DataModel.Model.GroupKey != null) {
		...
		private static DbExpression[] DoTypeCheck(DbExpression[] input, FieldDefinition[] output, SqlTransformStatement node)
		{
			...
			for (int i = 0; i < input.Length; ++i) {
				var value = input[i];
				var name = value is LiteralExpression ? output[i].Name : GetName(value);
		...
		private static void DoTypeCheck(DbExpression value, FieldDefinition field, SqlTransformStatement node)
		{
			...
			var typeResult = DataDictionaryComparer.TypeCheckField(inType, outType, value.ToString()!);
		...
		private static string GetName(DbExpression field) => field switch {
			...
			AggregateExpression ag => GetName(ag.Arg),
			CountExpression => "Count",
			_ => throw new NotImplementedException()
		};

		private static string? GetField(DbExpression field) => field switch {
			ReferenceExpression re => re.ToString(),
			AliasedExpression a => GetField(a.Expr),
			AggregateExpression ag => GetField(ag.Arg),
			CountExpression => "__Count__",
