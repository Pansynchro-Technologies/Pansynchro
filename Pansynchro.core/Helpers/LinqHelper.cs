...
namespace Pansynchro.Core.Helpers
{
	public static class LinqHelper
	{
				...
				IEqualityComparer<TKey>? keyComparer = null)
		{
				...
				[EnumeratorCancellation] CancellationToken cancellationToken = default)
			{
				...
				async IAsyncEnumerable<TSource> GetAdjacent(Tuple<TSource, TKey, bool> state)
				{
					...
					if (!ReferenceEquals(Interlocked.CompareExchange<Tuple<TSource, TKey, bool>>(
		...
		private static IEnumerable<T> WhereNotNullImpl<T>(IEnumerable<T?> source) where T : class
		{
			foreach (var item in source) {
				if (item != null) {
					yield return item;
